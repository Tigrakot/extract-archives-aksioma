...
async function extractWithSystem(archiveBuffer, archiveName) {
  ...
  try {
    ...
    if (/\.zip$/i.test(archiveName)) {
      console.log(`[EXTRACT] running: unzip ${archiveName}`);
      ...
      console.log(`[EXTRACT] running: 7z x ${archiveName}`);
      ...
      console.log(`[EXTRACT] running: 7z x ${archiveName} (rar)`);
  ...
  try {
    ...
    for (const archive of archives) {
      ...
      try {
        ...
        console.error(`[EXTRACT] task=${taskId} failed to process ${archive.name}:`, err.message, 'code:', err.code);
        // Помечаем архив как проблемный — будет упомянут в комментарии
        // ENOENT = бинарь (7z/unrar) не найден в системе
        if (err.code === 'ENOENT') {
          const ext = archive.name.match(/\.(\w+)$/)?.[1] || '?';
          archive.unsupported = ext;
        } else {
          // Любая другая ошибка — реальная проблема с архивом или утилитой
          archive.error = err.message || String(err);
        }
      }
    }
